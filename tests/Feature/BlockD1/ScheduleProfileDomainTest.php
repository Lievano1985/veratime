...
class ScheduleProfileDomainTest extends TestCase
{
    ...
    public function test_resolution_priority_uses_direct_unit_center_company_and_ignores_temporary_support(): void
    {
        ...
        $this->assertSame('UNIT', $resolver->handle($company, $relationship, '2026-08-05')['schedule_profile']->code);
    ...
    public function test_policies_allow_rh_operativo_to_create_profiles_and_assign_only_inside_center_scope(): void
    {
        $company = Company::factory()->create(['status' => 'active']);
        $center = Center::factory()->for($company)->create(['status' => 'active']);
        $otherCenter = Center::factory()->for($company)->create(['status' => 'active']);
        ...
        $this->assertTrue(Gate::forUser($rhOperativo)->allows('create', [ScheduleProfile::class, $company]));
        $this->assertFalse(Gate::forUser($rhOperativo)->allows('update', $profile));
        ...
        $this->assertTrue(Gate::forUser($rhOperativo)->allows('assignToCenter', [ScheduleProfile::class, $company, $center, '2026-08-01']));
        $this->assertTrue(Gate::forUser($rhOperativo)->allows('assignToUnit', [ScheduleProfile::class, $company, $unit, '2026-08-01']));
        $this->assertFalse(Gate::forUser($rhOperativo)->allows('assignToCenter', [ScheduleProfile::class, $company, $otherCenter, '2026-08-01']));

        $assignment = app(AssignScheduleProfileAction::class)->handle($company, $profile, [
            'assignment_scope' => 'center',
            'center_id' => $center->id,
            'effective_from' => '2026-08-01',
        ], $rhOperativo);
        $this->assertSame($center->id, $assignment->center_id);
