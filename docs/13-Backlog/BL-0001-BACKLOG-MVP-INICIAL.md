...
| F. Calendario diario y publicacion | Crear batches obligatorios por centro y daily schedules publicados | D/E | `schedule_batches`, `daily_schedule_assignments`, `daily_schedule_segments` | publicacion no destructiva | F1/F2/F3A/F3B implementados/candidatos a cierre: nucleo diario, generacion draft, publicacion atomica e interfaz de administracion; correcciones F4, CSV/API y calculos pendientes | Versionado incorrecto | 6 |
