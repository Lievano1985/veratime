...
Mejora operativa de Jornadas unifica `Actualizar` y `Calcular` en `Procesar jornadas`. La UI ejecuta `ProcessCompanyWorkDaysAction`, que refresca work_days, calcula versiones pendientes/stale y aplica motor legal disponible; el comando y el job automatico usan el mismo flujo.
