...
Nota Bloque F5B:
Se implementa la interfaz de importacion CSV dentro de `/scheduling/daily` para lotes `draft`. Permite descargar plantilla CSV version 1, cargar archivo privado, validar, revisar preview paginado, aplicar con confirmacion y hash vigente, cancelar importaciones y descargar errores. No incluye XLSX, API WFM, jobs asincronos, publicacion automatica, `work_days`, calculos legales, alertas, incidencias, cierres, conformidad ni reportes.
