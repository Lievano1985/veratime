...
Bloque Work Days refresco operativo agrega configuracion de hora automatica por empresa, ejecucion manual desde `/companies`, comandos `work-days:refresh` y `work-days:auto-refresh`, y scheduler cada minuto para refrescar empresas activas por hora local. No incluye `work_day_calculations`, motor legal, horas extra, alertas, incidencias, cierres, conformidad, reportes ni API.
