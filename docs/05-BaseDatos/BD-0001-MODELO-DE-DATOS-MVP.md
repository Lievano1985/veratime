...
`daily_schedule_segments`: segmentos congelados de una asignacion diaria. Campos: `company_id`, `daily_schedule_assignment_id`, `segment_order`, `segment_type` (`work`, `break`), `timing_mode` (`fixed`, `duration`), `start_local_time`, `end_local_time`, `start_day_offset`, `end_day_offset`, `starts_at_utc`, `ends_at_utc`, `duration_minutes`, `is_paid`, `is_required`, `shift_template_segment_id`, `metadata`, timestamps. Indices: unique `daily_schedule_assignment_id/segment_order`, index `company_id/daily_schedule_assignment_id`, index `company_id/segment_type`.
