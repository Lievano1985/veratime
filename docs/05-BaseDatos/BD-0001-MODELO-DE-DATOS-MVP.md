...
`schedule_profiles`: perfiles de horario. Bloque D1/D2 implementa `pattern` con `pattern_mode = weekly` y `calendar`. Campos: `company_id`, `code`, `name`, `description`, `profile_type`, `pattern_mode`, `status`, `metadata`. No incluye `timezone`, `center_id`, `worker_id`, `required_minutes` ni ventanas flexibles. Indices: unique `company_id/code`, index `company_id/profile_type`, index `company_id/profile_type/pattern_mode`, index `company_id/status`. `cycle`, `flexible` y `on_call` quedan preparados conceptualmente, pero no operativos todavia.
