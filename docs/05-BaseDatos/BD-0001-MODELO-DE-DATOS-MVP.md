...
`schedule_profiles`: perfiles de horario. Bloque D1 implementa solo `fixed` y `variable`. Campos: `company_id`, `code`, `name`, `description`, `profile_type`, `status`, `metadata`. No incluye `timezone`, `center_id`, `worker_id`, `required_minutes` ni ventanas flexibles. Indices: unique `company_id/code`, index `company_id/profile_type`, index `company_id/status`.

`schedule_profile_weekly_rules`: reglas semanales de perfiles `fixed`. Campos: `company_id`, `schedule_profile_id`, `day_of_week` ISO 1-7, `day_type` (`shift`, `rest`), `shift_template_id`, `metadata`. Indices: unique `schedule_profile_id/day_of_week`, index `company_id/schedule_profile_id`, index `company_id/shift_template_id`. Un perfil `fixed` requiere exactamente siete reglas y al menos un dia `shift`; un perfil `variable` no admite reglas semanales.
