...
class CreateMandatoryRestDayAction
{
    public function handle(?Company $company, array $data): MandatoryRestDay
    {
        ...
        $stateCode = $this->normalizeStateCode($data['state_code'] ?? null);

        $this->assertValidPayload($company, $type, $scope, $stateCode, $status, $captureSource, $data);

        $date = CarbonImmutable::parse($data['date'])->toDateString();

        return DB::transaction(function () use ($company, $data, $type, $scope, $stateCode, $status, $captureSource, $date): MandatoryRestDay {
            $this->assertUniqueMandatoryRestDay($type, $scope, $company, $stateCode, $date, $data['name']);
    ...
    private function assertUniqueMandatoryRestDay(string $type, string $scope, ?Company $company, ?string $stateCode, string $date, string $name): void
    {
