...
{
    public function handle(?Company $company, ?Center $center, array $data): MandatoryRestDay
    {
        ...
        return DB::transaction(function () use ($company, $center, $data, $scope, $status, $date): MandatoryRestDay {
            $this->assertUniqueMandatoryRestDay($scope, $company, $center, $date, $data['name']);
            ...
            $restDay->company()->associate($scope === 'global' ? null : $company);
    ...
    private function assertValidPayload(?Company $company, ?Center $center, ?string $scope, string $status, array $data): void
    {
        ...
        if (! in_array($scope, ['global', 'company', 'center'], true)) {
            throw new InvalidArgumentException('Mandatory rest day scope is invalid.');
        }
        ...
        if ($scope === 'global') {
            if ($company || $center) {
                throw new InvalidArgumentException('Global mandatory rest days cannot belong to a company or center.');
            }
        ...
        if (! $company) {
            throw new InvalidArgumentException('Company mandatory rest days require an active company.');
        }
    ...
    private function assertUniqueMandatoryRestDay(string $scope, ?Company $company, ?Center $center, string $date, string $name): void
    {
    ...
    {
        if ($scope === 'global') {
            $query->whereNull('company_id')->whereNull('center_id');
